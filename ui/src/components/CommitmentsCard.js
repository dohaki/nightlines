...
export default function CommitmentsCard() {
  ...
  useEffect(() => {
    function handleReceiveTransfer(event) {
      ...
      if (data.type === "transfer" && data.zkpPublicKey === zkpPublicKey) {
    ...
    return () =>
      webSocket.removeEventListener("message", handleReceiveTransfer);
  ...
  return (
    <DashboardCard>
      ...
      <Flex justifyContent={"space-between"} color={"primary"} mb={3}>
        <Text>{iouAbbreviation} Spendable:</Text>
        <Text fontWeight={"bold"}>{spendable}</Text>
      </Flex>
      <Flex mb={3}>
        <Label>
          <Radio
